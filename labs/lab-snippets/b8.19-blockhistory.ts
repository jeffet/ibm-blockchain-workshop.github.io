...
async function main() {
  try {
    ...
    const connectionProfile: string = path.resolve(
      __dirname,
      '..',
    ...
    await network.addBlockListener(
        ...
        if (block !== undefined) {
          ...
          for (const i in block.data.data) {
            if (block.data.data[i].payload.data.actions !== undefined) {
              const inputArgs: Buffer[] =
                block.data.data[i].payload.data.actions[0].payload
              ...
              const txTime = new Date(
                block.data.data[i].payload.header.channel_header.timestamp
              ).toUTCString();
              const txId: string =
                block.data.data[i].payload.header.channel_header.tx_id;
              ...
              for (let j = 0; j < inputArgs.length; j++) {
                const inputArgPrintable: string = inputArgs[j]
                  .toString()
                  .replace(/[^\x20-\x7E]+/g, '');
              ...
              for (const l in block.data.data[i].payload.data.actions[0].payload
                .action.proposal_response_payload.extension.results.ns_rwset[0]
                .rwset.writes) {
                keyData = keyData.concat(
                  block.data.data[i].payload.data.actions[0].payload.action
                    .proposal_response_payload.extension.results.ns_rwset[0]
                    .rwset.writes[l].key,
                  ' '
                );
              }
              ...
              for (const k in block.data.data[i].payload.data.actions[0].payload
                .action.endorsements) {
                endorsers = endorsers.concat(
                  block.data.data[i].payload.data.actions[0].payload.action
                    .endorsements[k].endorser.Mspid,
                  ' '
                );
              }
      ...
      { startBlock: 1 }
    ); // Read from block 1, and continue to catch new blocks as they appear
