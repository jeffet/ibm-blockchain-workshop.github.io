...
async function main() {
  try {
    // The ID of the car to look up (can be overridden by the first commandline argument)
    ...
    const carDetailsBuffer = await contract.evaluateTransaction(
      'queryCar',
      carid
    );
    const car: Car = JSON.parse(carDetailsBuffer.toString()) as Car;
    const previousOwnersBuffer = await contract.evaluateTransaction(
      'getPreviousOwners',
      carid
    );
    const history: History = JSON.parse(
      previousOwnersBuffer.toString()
    ) as History;
    console.log(
      `Owner history of ${carid} (currently ${car.color} ${car.make} ${car.model}):`
    );

    // Display the previous owners
    if (history.previousOwnerCount > 0) {
      for (let i = 0; i < history.previousOwners.length; i++) {
        const txTime = new Date(
          history.previousOwnershipChangeDates[i]
        ).toUTCString();
        if (history.previousOwners[i] === DELETED_CAR_OWNER_IDENTIFIER) {
          ...
          console.log(
            `${txTime}: ${
              history.previousOwners[i]
            } became owner #${++ownerSequenceNum}`
          );
        }
      }
    }
    ...
    console.log(
      `${txTime}: ${history.currentOwner} became owner #${++ownerSequenceNum}`
    );
